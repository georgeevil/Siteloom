...
| `DTZ` | 106 | **The one with real correctness weight.** The store *is* naive-UTC by contract now (CLD-100, `store/models._utcnow`), which reframes the survey: the 121× `DTZ001` in tests are naive literals in the contract's own storage shape (they would want `"tests/*" = ["DTZ001"]`), and `siteloom/` itself is down to five hits. One (`web/library_routes.py` `day_start`) is contract-compliant internal UTC arithmetic that wants a `noqa`. The other four are `DTZ006` `fromtimestamp()` without `tz` — genuine boundary findings that read the *server's local* wall clock into naive-UTC columns — but not one-liners: the library indexer compares stored mtimes to fresh ones to detect changed files, so changing its epoch basis makes every already-indexed item on a non-UTC host read as modified on the next scan (a re-index storm), and the resolver's pending-pool pair (`resolver.py` write ~329 / read ~144) is self-consistently local on both sides, so both must move together with persisted payloads in mind. Enable after those four are fixed with their migrations thought through — not as a side effect of another change. |
